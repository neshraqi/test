...
import com.hami.common.BaseController.AppliedFilters;
...
public class NewDesignFilterTrainFragmentDialog extends AAH_FabulousFragment {
    ...
    public final static String FILTER_CATEGORY_TRAIN_TYPE = "fctt";
    public final static String FILTER_CATEGORY_TIME_TAKE_OFF = "fctto";
    public final static String FILTER_CATEGORY_TIME_LANDING = "fctl";
    public final static String FILTER_CATEGORY_TRAIN_COMPANY = "fctc";
    ...
    //-----------------------------------------------
    public static NewDesignFilterTrainFragmentDialog newInstance(Callbacks callbacks) {
        NewDesignFilterTrainFragmentDialog.callbacks = callbacks;
        NewDesignFilterTrainFragmentDialog mff = new NewDesignFilterTrainFragmentDialog();
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            trainCompanyHashMap = (HashMap<String, String>) savedInstanceState.getSerializable("trainCompanyHashMap");
    ...
    public void onSaveInstanceState(Bundle outState) {
        if (outState != null) {
            outState.putSerializable("trainCompanyHashMap", trainCompanyHashMap);
    ...
    //-----------------------------------------------
    public void setTrain(HashMap<String, String> trainCompanyHashMap) {
        try {
            this.trainCompanyHashMap = trainCompanyHashMap;
        } catch (Exception e) {
    ...
    public void setupDialog(Dialog dialog, int style) {
        ...
        //if (contentView == null)
        contentView = View.inflate(getContext(), R.layout.fragment_train_dialog_filter_view, null);
        resetFilterView();
    ...
    private void resetFilterView() {
        ...
        sort(rl_content);
        price(rl_content);
        setTypeTrain(rl_content);
        setupTimeTakeOff(rl_content);
        setupTimeLanding(rl_content);
        setupTrainCompany(rl_content);
    ...
    private void sort(View contentView) {
        ...
        final ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(getActivity(), R.array.sortTrain, R.layout.row_item_spinner_train_white);
    ...
    //-----------------------------------------------
    private void setTypeTrain(View contentView) {
        LinearLayout linearLayout = (LinearLayout) contentView.findViewById(R.id.layoutTrainType);
        final ArrayList<String> listTypeTrain = new ArrayList<>();
        listTypeTrain.add("سالنی");
        listTypeTrain.add("کوپه ای");
        for (int index = 0; index < listTypeTrain.size(); index++) {
            final CheckBox checkBox = new CheckBox(getActivity());
            checkBox.setTag(index);
            Integer tag = (Integer) checkBox.getTag();
            if (applied_filters != null &&
                    applied_filters.get(FILTER_CATEGORY_TRAIN_TYPE) != null &&
                    applied_filters.get(FILTER_CATEGORY_TRAIN_TYPE).contains(String.valueOf(tag))) {
                checkBox.setCheck(true);
            } else {
            ...
            checkBox.setCallBack(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked)
                        addToSelectedMap(FILTER_CATEGORY_TRAIN_TYPE, String.valueOf(checkBox.getTag()));
                    else
                        removeFromSelectedMap(FILTER_CATEGORY_TRAIN_TYPE, String.valueOf(checkBox.getTag()));


                }
            });
            checkBox.setTitle(listTypeTrain.get(index));
    ...
    //-----------------------------------------------
    private void setupTimeLanding(View contentView) {
        LinearLayout linearLayout = (LinearLayout) contentView.findViewById(R.id.layoutTimeLanding);
        ArrayList<String> listStops = new ArrayList<>();
        listStops.add(getString(R.string.sortByTimeTitleMorning) + "(" + getString(R.string.sortByTimeValueMorning) + ")");
        listStops.add(getString(R.string.sortByTimeTitleNoon) + "(" + getString(R.string.sortByTimeValueNoon) + ")");
        listStops.add(getString(R.string.sortByTimeTitleAfternoon) + "(" + getString(R.string.sortByTimeValueAfternoon) + ")");
        ...
        for (int index = 0; index < listStops.size(); index++) {
            ...
            Integer tag = (Integer) checkBox.getTag();
            if (applied_filters != null &&
                    applied_filters.get(FILTER_CATEGORY_TIME_LANDING) != null &&
                    applied_filters.get(FILTER_CATEGORY_TIME_LANDING).contains(String.valueOf(tag))) {
                checkBox.setCheck(true);
            } else {
                checkBox.setCheck(false);
            }
            checkBox.setCallBack(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked)
                        addToSelectedMap(FILTER_CATEGORY_TIME_LANDING, String.valueOf(checkBox.getTag()));
                    else
                        removeFromSelectedMap(FILTER_CATEGORY_TIME_LANDING, String.valueOf(checkBox.getTag()));
    ...
    //-----------------------------------------------
    private void setupTrainCompany(View contentView) {
        LinearLayout linearLayout = (LinearLayout) contentView.findViewById(R.id.layoutTrainCompany);
        Iterator<Map.Entry<String, String>> iterator = trainCompanyHashMap.entrySet().iterator();
        while (iterator.hasNext()) {
            ...
            if (applied_filters != null &&
                    applied_filters.get(FILTER_CATEGORY_TRAIN_COMPANY) != null &&
                    applied_filters.get(FILTER_CATEGORY_TRAIN_COMPANY).contains(tag)) {
                checkBox.setCheck(true);
            } else {
                checkBox.setCheck(false);
            }
            checkBox.setCallBack(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked)
                        addToSelectedMap(FILTER_CATEGORY_TRAIN_COMPANY, String.valueOf(checkBox.getTag()));
                    else
                        removeFromSelectedMap(FILTER_CATEGORY_TRAIN_COMPANY, String.valueOf(checkBox.getTag()));
